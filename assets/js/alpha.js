$(document).ready(function(){
    ...
    $('#exampleModal').on('show.bs.modal', function (event) {
        ...
        $.getJSON('characters.json', function(data) {
            ...
            modal.find('.modal-leader-skill>li').text(_leader_skill)

            modal.find('.modal-skill-ultimate>li').text(_skill_ultimate.skill_description)
            modal.find('.modal-skill-1>li').text(_skill_1.skill_description)
            modal.find('.modal-skill-2>li').text(_skill_2.skill_description)
            modal.find('.modal-skill-3>li').text(_skill_3.skill_description)
            if(data[id].skill[4])
                modal.find('.modal-skill-4>li').text(_skill_4.skill_description)

            modal.find('.modal-skill-ultimate-name').text(_skill_ultimate.skill_name)
            modal.find('.modal-skill-1-name').text(_skill_1.skill_name)
            modal.find('.modal-skill-2-name').text(_skill_2.skill_name)
            modal.find('.modal-skill-3-name').text(_skill_3.skill_name)
            if(data[id].skill[4])
                modal.find('.modal-skill-4-cooldown').text("(Max Cooldown : "+_skill_4.max_cooldown+")")

            modal.find('.modal-skill-ultimate-cooldown').text("(Max Cooldown : "+_skill_ultimate.max_cooldown+")")
            modal.find('.modal-skill-1-cooldown').text("(Max Cooldown : "+_skill_1.max_cooldown+")")
            modal.find('.modal-skill-2-cooldown').text("(Max Cooldown : "+_skill_2.max_cooldown+")")
            modal.find('.modal-skill-3-cooldown').text("(Max Cooldown : "+_skill_3.max_cooldown+")")
            if(data[id].skill[4])
                modal.find('.modal-skill-4-cooldown').text("(Max Cooldown : "+_skill_4.max_cooldown+")")
